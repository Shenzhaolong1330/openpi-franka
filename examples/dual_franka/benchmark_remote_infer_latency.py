...
def make_dummy_observation() -> dict:
    """Create a dummy observation for testing dual franka policy.
    
    State is 28D: [left_joint(7), right_joint(7), left_ee(6), right_ee(6), left_gripper(1), right_gripper(1)]
    """
    state = np.random.rand(28).astype(np.float32)
...
def benchmark_remote_inference(client, num_warmup: int = 3, num_iterations: int = 50):
    """Benchmark remote inference latency for dual franka.
    ...
    print(f"\n{'='*60}")
    print(f"Remote Inference Latency Benchmark (Dual Franka)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark remote OpenPI inference latency for Dual Franka")
